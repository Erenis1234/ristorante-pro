...
`preload.js` espone `window.api` al renderer via `contextBridge` (context isolation attiva, `nodeIntegration: false`). Ogni dominio ha un proprio namespace — `window.api.menu`, `.menuBuilder`, `.magazzino`, `.ricette`, `.ordini`, `.foodcost`, `.personale`, `.auth`, `.sync` — mappato 1:1 sui canali IPC registrati nei rispettivi `ipc/*.ipc.js`.
