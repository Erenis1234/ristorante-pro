...
function registerAuthIpcHandlers() {
	...
	registerHandler('ipc-prepare-password-reset', async (_event, recoveryUrl) => auth.preparePasswordReset(recoveryUrl))
	registerHandler('ipc-complete-password-reset', async (_event, newPassword) => auth.completePasswordReset(newPassword))
	registerHandler('ipc-cancel-password-reset', async () => auth.cancelPasswordReset())
